...
constructor() {
  ...
  setDefaultMemberPermissions(perm) { this._defaultMemberPermissions = perm; return this; }
  ...
  addOption(fn) {
    ...
    this._optionsBuilder = prev ? (b) => { prev(b); fn(b); } : fn;
    return this;
  }
   ...
   */
  addUserOption(fn) { return this.addOption((b) => b.addUserOption(fn)); }
  addStringOption(fn) { return this.addOption((b) => b.addStringOption(fn)); }
  addIntegerOption(fn) { return this.addOption((b) => b.addIntegerOption(fn)); }
  addNumberOption(fn) { return this.addOption((b) => b.addNumberOption(fn)); }
  addBooleanOption(fn) { return this.addOption((b) => b.addBooleanOption(fn)); }
  addChannelOption(fn) { return this.addOption((b) => b.addChannelOption(fn)); }
  addRoleOption(fn) { return this.addOption((b) => b.addRoleOption(fn)); }
  ...
  onModal(localName, handler) { this._modals.set(localName, handler); return this; }
  onAutocomplete(optionName, handler) { this._autocomplete.set(optionName, handler); return this; }
  addPrecondition(fn) { if (typeof fn === "function") this._preconditions.push(fn); return this; }

  /**
   * Build SlashCommand JSON for registration.
   */
  toSlashJson() {
    const builder = new SlashCommandBuilder().setName(this._name).setDescription(this._description || " ");
  ...
  textInput(customId, label, style = TextInputStyle.Paragraph, required = true) {
    return new TextInputBuilder().setCustomId(customId).setLabel(label).setStyle(style).setRequired(required);
  }
}
...
function buildArgs(interaction) {
  ...
  try {
    ...
    for (const opt of data) {
      ...
      switch (opt.type) {
        case 3: out[name] = interaction.options.getString(name); break;
        case 4: out[name] = interaction.options.getInteger(name); break;
        case 10: out[name] = interaction.options.getNumber(name); break;
        case 5: out[name] = interaction.options.getBoolean(name); break;
        case 6: out[name] = interaction.options.getUser(name); break;
        case 7: out[name] = interaction.options.getChannel(name); break;
        case 8: out[name] = interaction.options.getRole(name); break;
        case 9: out[name] = interaction.options.getMentionable(name); break;
        case 11: out[name] = interaction.options.getAttachment(name); break;
        default: out[name] = val?.value ?? null;
      }
    }
  } catch {}
  return out;
...
export function createBuilderRegistry() {
  ...
  function add(moduleName, builder) {
    ...
    if (!set) { set = new Set(); byModule.set(moduleName, set); }
    set.add(builder);
