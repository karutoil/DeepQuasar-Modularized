...
import { getGuildSettings, setGuildSettings, validateRoleAssignable } from "../services/settings.js";
...
export function createConfigureCommand(ctx) {
  ...
  const cmd = v2.createInteractionCommand()
    ...
    .onExecute(async (interaction) => {
      try {
        ...
        lifecycle.addDisposable(() => { try { clearSession(msg.id); } catch {} });
      } catch (e) {
        logger.warn("[Autorole] open configure failed", { error: e?.message || e });
        try { await interaction.reply({ content: "Error opening configuration.", ephemeral: true }); } catch {}
      }
    });

  const off = v2.register(cmd);
  lifecycle.addDisposable(() => { try { off?.(); } catch {} });
  ...
  const offRole = interactions.registerSelect(moduleName, "ar_role_select", async (interaction) => {
    ...
    if (chosenRoleId) {
      ...
      if (!v.ok) {
        return interaction.reply({ content: `Cannot assign that role: ${v.reason}`, ephemeral: true });
      }
    }
  ...
  const buttonIds = [
    "ar_delay_none", "ar_delay_10s", "ar_delay_60s", "ar_delay_5m", "ar_delay_custom",
    "ar_toggle_ignore_bots", "ar_toggle_age_gate", "ar_age_days_custom",
    "ar_save", "ar_cancel"
  ];
      ...
      try {
        ...
        if (id.startsWith("ar_delay_")) {
          ...
          if (id === "ar_delay_custom") {
            const modal = new ModalBuilder().setTitle("Custom Delay (seconds)").setCustomId("ar_modal_delay");
        ...
        if (id === "ar_toggle_age_gate") {
          const wasEnabled = cur.minAccountAgeDays != null && Number.isFinite(cur.minAccountAgeDays);
        ...
        if (id === "ar_age_days_custom") {
          const modal = new ModalBuilder().setTitle("Minimum Account Age (days)").setCustomId("ar_modal_age_days");
          const input = new TextInputBuilder()
        ...
        if (id === "ar_save") {
          ...
          if (!cur.roleId) {
            return interaction.reply({ content: "Please select a role before saving.", ephemeral: true });
          }
          ...
          try { ctx.autorole?.invalidate?.(cur.guildId); } catch {}
          const savedView = buildConfigEmbed(interaction.guild, { ...toSave });
          // Clear session on save
          clearSession(msgId);
          return interaction.update({ embeds: [savedView.embed.setFooter({ text: "Saved ✔" })], components: savedView.components });
        }
  ...
  // Modal handlers
  const offModalDelay = interactions.registerModal(moduleName, "ar_modal_delay", async (interaction) => {
    if (!interaction.inCachedGuild()) return;
    if (!interaction.member?.permissions?.has?.(PermissionsBitField.Flags.ManageGuild)) {
      return interaction.reply({ content: "Missing permission.", ephemeral: true });
    }
    ...
    if (!Number.isFinite(secs) || secs < 0 || secs > 86400) {
      return interaction.reply({ content: "Invalid delay. Enter a number between 0 and 86400.", ephemeral: true });
    }
  ...
  const offModalAge = interactions.registerModal(moduleName, "ar_modal_age_days", async (interaction) => {
    if (!interaction.inCachedGuild()) return;
    if (!interaction.member?.permissions?.has?.(PermissionsBitField.Flags.ManageGuild)) {
      return interaction.reply({ content: "Missing permission.", ephemeral: true });
    }
    ...
    if (!Number.isFinite(days) || days < 0) {
      return interaction.reply({ content: "Invalid number of days. Provide an integer >= 0.", ephemeral: true });
    }
...
function buildConfigEmbed(guild, settings) {
  ...
  const ageGateEnabled = settings.minAccountAgeDays != null && Number.isFinite(settings.minAccountAgeDays);
  const ageGateText = ageGateEnabled ? `${settings.minAccountAgeDays} day(s)` : "Disabled";

  const embed = new EmbedBuilder()
    .setTitle("Autorole Configuration")
    .setDescription("Configure the role assignment behavior for new members.\nOnly users with Manage Guild can use this.")
    .addFields(
  ...
  const delayRow = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId("ar_delay_none").setLabel("No Delay").setStyle(ButtonStyle.Secondary),
    ...
    new ButtonBuilder().setCustomId("ar_delay_custom").setLabel("Custom…").setStyle(ButtonStyle.Primary),
  );
