...
export function registerChatCommand(ctx) {
  ...
  const cmd = v2
    ...
    .setDefaultMemberPermissions(PermissionsBitField.Flags.Administrator) // already set
  ...
  cmd.addOption((root) => {
    root.addSubcommandGroup((group) =>
      group
        ...
        .addSubcommand((sub) =>
          sub
            ...
            .addStringOption((opt) =>
              opt
                .setName('key')
                .setDescription('Configuration key to get.')
                .setRequired(true)
                .addChoices(
                  { name: 'API Key', value: 'apiKey' },
                  { name: 'Base URL', value: 'baseUrl' },
        ...
        .addSubcommand((sub) =>
          sub.setName('list').setDescription('List all configuration values for this server.')
        )
    );
  });
    ...
    try {
      if (group === 'config') {
        ...
        if (sub === 'set') {
          switch (key) {
            ...
            case 'historyLimit':
              ...
              if (isNaN(value) || value < 0) {
                return interaction.reply({
                  content: 'History limit must be a non-negative integer.',
                  ephemeral: true,
                });
              }
              break;
            case 'activeChannel':
              if (value.toLowerCase() === 'none') {
                ...
                if (m) {
                  ...
                  return interaction.reply({
                    content: 'Invalid channel mention or ID.',
                    ephemeral: true,
                  });
                }
              }
          ...
          return interaction.reply({
            content: `Configuration ${key} set to ${value === null ? 'none' : value}.`,
            ephemeral: true,
          });
        }
        if (sub === 'get') {
          ...
          return interaction.reply({
            content: `Current ${key}: ${val === null ? 'none' : val}.`,
            ephemeral: true,
          });
        }
        if (sub === 'list') {
          const all = await getAllGuildSettings(ctx, interaction.guildId);
          const fields = Object.entries(all).map(([k, v]) => ({
            name: k,
            value: `\`${v === null ? 'none' : v}\``,
            inline: true,
          }));
          const em = embed.neutral({
