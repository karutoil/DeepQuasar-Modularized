...
export default async function init(ctx) {
  ...
  cmdModlog.addOption((root) => {
    root.addSubcommand((sub) =>
      sub.setName("search")
        ...
        .addStringOption(opt => opt.setName("event").setDescription("Event type or alias (or 'all')").setRequired(false).setAutocomplete(true))
        .addUserOption(opt => opt.setName("executor").setDescription("Requestor (executor) user").setRequired(false))
        .addUserOption(opt => opt.setName("target").setDescription("Actioned (target) user").setRequired(false))
        .addChannelOption(opt => opt.setName("channel").setDescription("Channel reference").setRequired(false))
        .addRoleOption(opt => opt.setName("role").setDescription("Role reference").setRequired(false))
        .addStringOption(opt => opt.setName("emoji").setDescription("Emoji ID").setRequired(false))
        .addStringOption(opt => opt.setName("webhook").setDescription("Webhook ID").setRequired(false))
        .addStringOption(opt => opt.setName("integration").setDescription("Integration ID").setRequired(false))
        .addStringOption(opt => opt.setName("since").setDescription("ISO datetime or relative (e.g., 7d,4h)").setRequired(false))
        .addStringOption(opt => opt.setName("until").setDescription("ISO datetime or relative (e.g., now, 1d)").setRequired(false))
        .addStringOption(opt => opt.setName("reason").setDescription("Reason contains text").setRequired(false))
        .addIntegerOption(opt => opt.setName("page").setDescription("Page to open").setRequired(false))
        .addIntegerOption(opt => opt.setName("page_size").setDescription("Results per page").setRequired(false))
    );
  });
  ...
  cmdModlog.addOption((root) => {
    root.addSubcommand((sub) =>
      sub.setName("export")
        ...
        .addStringOption(opt =>
          opt.setName("format").setDescription("Export format").setRequired(true)
            .addChoices(
              { name: "csv", value: "csv" },
  ...
  cmdModlog.addOption((root) => {
    root.addSubcommand((sub) =>
      sub.setName("inspect")
        ...
        .addStringOption(opt => opt.setName("id").setDescription("Audit log entry ID (snowflake)").setRequired(true))
    );
  });
  ...
  cmdModlog.onAutocomplete("event", async (interaction) => {
    try {
      ...
      const focused = interaction.options?.getFocused?.(true) || interaction.options?.getFocused?.() || "";
      ...
      try {
        ...
        try { await interaction.respond([{ name: "all", value: "all" }]); } catch {}
      }
    } catch (e) {
      // Final fallback
      try { await interaction.respond([{ name: "all", value: "all" }]); } catch {}
      try { mod.logger?.warn?.("[ModLog] Autocomplete failed, responded with fallback", { error: e?.message }); } catch {}
    }
  });

  // Register pagination button using interactions with prefix
  const searchHandler = createSearchHandler(mod);
  const offButton = interactions.registerButton(moduleName, "modlog:page:", async (interaction) => {
