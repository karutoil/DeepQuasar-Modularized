...
export function createBanCommand(ctx) {
  ...
  const cmdBan = v2.createInteractionCommand()
    .setName("ban")
  ...
  try {
    ...
    if (subcommand === "add") {
      ...
      try {
        ...
        return interaction.editReply({ embeds: [embed.error("Failed to resolve target user for ban.")] });
      }
      logger.debug("[Moderation] Ban subcommand: target", { targetObj: target, targetId: target?.id, reason });

      try {
        // Try to get the member from the interaction options (preferred)
        member = interaction.options.getMember
          ? interaction.options.getMember("user")
          : undefined;
        logger.debug("[Moderation] Ban subcommand: member from options", { found: !!member, memberObj: member });

        // Fallback: fetch from guild if not present
        if (!member) {
          logger.debug("[Moderation] Ban subcommand: member not in options, fetching from guild...", { targetId: target?.id });
          try {
            member = await interaction.guild.members.fetch(target.id);
            logger.debug("[Moderation] Ban subcommand: member fetched from guild", { found: !!member, memberObj: member });
          } catch (fetchErr) {
            logger.warn("[Moderation] Ban failed: Could not fetch member from guild", { targetId: target?.id, error: fetchErr });
            return interaction.editReply({ embeds: [embed.error("User not found in this server (not cached and fetch failed).")] });
          }
        }
      } catch (err) {
        logger.error("[Moderation] Exception looking up member in guild", { error: err });
        return interaction.editReply({ embeds: [embed.error("Failed to resolve member in guild for ban.")] });
      }

      if (!member) {
        logger.warn("[Moderation] Ban failed: User not found in this server.", { targetId: target?.id });
      ...
      try {
        ...
        await interaction.editReply({ embeds: [embed.error(`Failed to ban user: ${err.message}`)] });
      }
      ...
      try {
        ...
        await interaction.editReply({ embeds: [embed.error(`Failed to unban user: ${err.message}`)] });
      }
    }
  } catch (outerErr) {
    logger.error("[Moderation] handleBan outer error", { error: outerErr });
    try {
      await interaction.editReply({ embeds: [embed.error(`Unexpected error: ${outerErr.message}`)] });
    } catch {}
  }
}